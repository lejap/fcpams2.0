...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resolve'])) {
    ...
    $name   = $_SESSION['name'];
    $stmt = $conn->prepare("UPDATE tickets SET status='RESOLVED', admin_remark=?, resolved_at=NOW(), resolved_by_name=? WHERE id=? AND status='OPEN'");
    $stmt->bind_param("ssi", $remark, $name, $tid);
    $stmt->execute();
    header("Location: submissions.php?view=$tid&resolved=1");
    exit();
}
...
<div class="fade-in">
    ...
    <div style="background:rgba(16,185,129,0.1);border:1px solid #10b981;color:#065f46;padding:1rem;border-radius:0.5rem;margin-bottom:1.5rem;">
        <i class="fas fa-check-circle"></i> Submission resolved successfully.
    </div>
    <?php endif; ?>
    ...
    <div style="display:grid;grid-template-columns:1fr 1.5fr;gap:1.5rem;">
        <div class="glass-card">
            ...
            if (!empty($detail['option_label'])) {
                $info_items[ucfirst(strtolower($detail['type'])) . ' Type'] = $detail['option_label'];
            }
        ...
        <div>
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="admin_remark" class="form-input" rows="5"
                                  placeholder="Describe the actions taken or provide a response to this submission..."
                    ...
                    <div style="background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.25);padding:1rem;border-radius:0.75rem;margin-bottom:1.5rem;">
                        <p style="font-size:0.85rem;color:#92400e;margin:0;">
                            ...
                            <strong>Note:</strong> Once submitted, this ticket will be marked as <strong>RESOLVED</strong>.
                        </p>
                    </div>
            ...
            <div class="glass-card" style="border-left:4px solid #8b5cf6;">
                <h5 style="color:#6d28d9;margin-bottom:0.5rem;"><i class="fas fa-lock"></i> Closed &amp; Confirmed by Admin</h5>
                <p style="font-size:0.85rem;color:#5b21b6;margin-bottom:0;">On <?php echo $detail['confirmed_at'] ? date('M d, Y H:i', strtotime($detail['confirmed_at'])) : '-'; ?></p>
            </div>
            <?php endif; ?>
        </div>
    </div>
