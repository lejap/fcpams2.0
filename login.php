...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT u.id, u.name, u.password_hash, u.role, u.branch_id, u.is_approved,
                                    COALESCE(b.is_ho, 0) AS is_ho_branch, b.name AS branch_name
                             FROM users u
    ...
    if ($user = $result->fetch_assoc()) {
        if (password_verify($password, $user['password_hash'])) {
            ...
            if (!$user['is_approved']) {
                ...
                $_SESSION['is_ho']      = (int)$user['is_ho_branch'];
