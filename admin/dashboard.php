...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_spam_complaint'])) {
    $cid = (int)$_POST['mark_spam_complaint'];
    $conn->query("UPDATE complaints SET status='SPAM' WHERE id=$cid");
    header("Location: dashboard.php");
    exit();
}
...
$cmp_where = "WHERE status = 'OPEN'";
if ($date_from) $cmp_where .= " AND DATE(created_at) >= '$date_from'";
if ($date_to)   $cmp_where .= " AND DATE(created_at) <= '$date_to'";

$submissions = $conn->query("SELECT * FROM tickets $sub_where ORDER BY created_at ASC");
...
<div class="fade-in">
    <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">
        <div>
            ...
            <p style="font-size:0.88rem;color:#64748b;">Active &amp; pending items requiring attention.</p>
        </div>
        <a href="closed.php" style="display:inline-flex;align-items:center;gap:0.5rem;padding:0.5rem 1.25rem;border-radius:0.75rem;background:rgba(139,92,246,0.08);border:1px solid rgba(139,92,246,0.25);color:#7c3aed;font-weight:600;font-size:0.88rem;text-decoration:none;">
            ...
            <span style="background:#8b5cf6;color:white;border-radius:9px;min-width:18px;height:18px;font-size:0.65rem;font-weight:800;display:inline-flex;align-items:center;justify-content:center;padding:0 4px;"><?php echo $resolved_tickets + $resolved_comp; ?></span>
        </a>
    </div>
    ...
    <div class="stat-card-row">
        ...
        <div class="stat-card">
            <div class="stat-card-title">Pending Users</div>
                ...
                <tr>
                    ...
                    <td style="display:flex;gap:0.3rem;">
                        ...
                        <form method="POST" style="margin:0;" onsubmit="return confirm('Mark this complaint as SPAM?');">
                            <input type="hidden" name="mark_spam_complaint" value="<?php echo $c['id']; ?>">
                            <button type="submit" class="btn btn-outline" style="padding:.2rem .6rem;font-size:.8rem;border-radius:2rem;color:#ef4444;border-color:#ef4444;">Spam</button>
                        </form>
                    </td>
                </tr>
                <?php endwhile; ?>
