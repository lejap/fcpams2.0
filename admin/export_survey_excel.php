...
/**
 * Escape text for XML cell value and remove invalid XML 1.0 control characters.
 */
...
/**
 * Build one <row> of inline-string + number cells safely.
 */
function buildRow(int $rowNum, array $cells): string {
    ...
    foreach ($cells as $ci => $cell) {
        ...
        if ($val === '' || $val === null) {
            ...
            $sIdx = ($type === 'h') ? ' s="1"' : '';
            $xml .= "<c r=\"$ref\" t=\"inlineStr\"$sIdx><is><t xml:space=\"preserve\">" . xesc((string)$val) . "</t></is></c>";
        }
    }
    $xml .= "</row>";
    return $xml;
...
function buildSummarySheet(array $survey, array $question_stats, int $response_count, $overall_csat, string $f_branch = '', string $f_from = '', string $f_to = ''): string {
    ...
    $rows .= buildRow($r++, [['s', 'Author: Lejap']]);
...
// xl/styles.xml  (complete valid Open XML style declaration)
