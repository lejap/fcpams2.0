...
// Handle admin confirm/close from this page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_ticket'])) {
    ...
    $name = $_SESSION['name'];
    $stmt = $conn->prepare("UPDATE tickets SET status='CLOSED', confirmed_at=NOW(), confirmed_by_name=? WHERE id=? AND status='RESOLVED'");
    $stmt->bind_param("si", $name, $tid);
    $stmt->execute();
    header("Location: closed.php?tab=submissions");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_complaint'])) {
    ...
    $name = $_SESSION['name'];
    $stmt = $conn->prepare("UPDATE complaints SET status='CLOSED', confirmed_at=NOW(), confirmed_by_name=? WHERE id=? AND status='RESOLVED'");
...
$cmp_where = "WHERE status IN ('RESOLVED','CLOSED')";
...
$total_cmp_closed  = $conn->query("SELECT COUNT(*) as c FROM complaints WHERE status='CLOSED'")->fetch_assoc()['c'];
$total_cmp_res     = $conn->query("SELECT COUNT(*) as c FROM complaints WHERE status='RESOLVED'")->fetch_assoc()['c'];
...
<div class="fade-in">
    ...
    <div style="margin-bottom:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">
        <div>
            ...
            <p style="font-size:0.88rem;color:#64748b;">Submissions and complaints that have been resolved and confirmed.</p>
        </div>
    </div>
        ...
        <table class="admin-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $sub['confirmed_by_name'] ? '<span style="background:#ede9fe;color:#5b21b6;padding:.15rem .5rem;border-radius:.25rem;font-size:.75rem;font-weight:600;">'.htmlspecialchars($sub['confirmed_by_name']).'</span>' : '<span style="color:#94a3b8;">-</span>'; ?></td>
                    <td style="display:flex;gap:0.3rem;flex-wrap:wrap;">
                        ...
                        <?php if ($sub['status']==='RESOLVED'): ?>
                        <form method="POST" style="margin:0;" onsubmit="return confirm('Confirm and close this submission?');">
                            <input type="hidden" name="ticket_id" value="<?php echo $sub['id']; ?>">
                            <button type="submit" name="confirm_ticket" class="btn btn-outline" style="padding:.2rem .6rem;font-size:.8rem;border-radius:2rem;color:#7c3aed;border-color:#7c3aed;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="display:flex;gap:0.3rem;flex-wrap:wrap;">
                        ...
                        <form method="POST" style="margin:0;" onsubmit="return confirm('Confirm and close this complaint?');">
                            <input type="hidden" name="complaint_id" value="<?php echo $c['id']; ?>">
                            <button type="submit" name="confirm_complaint" class="btn btn-outline" style="padding:.2rem .6rem;font-size:.8rem;border-radius:2rem;color:#7c3aed;border-color:#7c3aed;">
                                <i class="fas fa-lock"></i> Confirm &amp; Close
                            </button>
                        </form>
                        <?php elseif ($c['status']==='CLOSED'): ?>
                        <a href="complaint_resolution_doc.php?id=<?php echo $c['id']; ?>" target="_blank" class="btn btn-outline" style="padding:.2rem .6rem;font-size:.8rem;border-radius:2rem;color:#0e83b5;border-color:#0e83b5;">
                        ...
                        <form method="POST" enctype="multipart/form-data" style="margin:0;display:inline-flex;gap:0.25rem;align-items:center;">
                            ...
                            <input type="file" name="signed_doc" accept=".jpg,.jpeg,.png,.pdf"
                                   style="font-size:.72rem;max-width:110px;" required title="Upload signed document">
